...
export function GitHubWindow() {
  ...
  return (
    <Window id="github" title="GitHub Activity" width={520}>
      <div className="mb-3 flex items-center justify-between gap-2">
        <p className="font-mono text-[11px] text-win-muted">
      ...
      {loading && (
        <div className="flex items-center gap-2 py-8 font-mono text-xs text-win-muted">
      ...
      {error && (
        <div className="border border-win-danger/30 bg-win-danger/5 p-3 font-mono text-xs text-win-danger">
          {error}
      ...
      {!loading && !error && events.length === 0 && (
        <p className="py-6 font-mono text-xs text-win-muted">No recent public events.</p>
      )}
      ...
      <ul className="space-y-2">
        {events.map((event) => (
          <li
            key={event.id}
            className="flex gap-2 border border-win-border/80 bg-win-bg/40 px-3 py-2"
          >
            <GitCommitHorizontal className="mt-0.5 h-3.5 w-3.5 shrink-0 text-win-accent" />
            <div className="min-w-0">
              <p className="truncate font-mono text-xs text-win-text">{labelFor(event)}</p>
              <p className="font-mono text-[10px] text-win-muted">
